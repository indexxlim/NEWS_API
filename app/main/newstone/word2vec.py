...
class FlaskWord2Vec(object):
    ...
    def init_app(self, app):
        ...
        #     self._wv[app.config['WORD2VEC_FILE']] = Word2Vec.KeyedVectors.load_word2vec_format(app.config['WORD2VEC_FILE'], 
        #                         binary=app.config['WORD2VEC_FILE_BINARY'])
        # else:
        #     self._wv[app.config['WORD2VEC_FILE']] = Word2Vec.load(app.config['WORD2VEC_FILE'])


        word2vec_path = app.config['word2vec_path']


        self._wv[app.config['WORD2VEC_FILE']] = Word2Vec.load(word2vec_path)  # Pre-Trained Word2Vec
