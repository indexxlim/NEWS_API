...
def get_news(n_url):
    ...
    #분류명
    try:
        aclass = bsoup.select('em.guide_categorization_item')[0].get_text()
    except:
        aclass = ''
...
def search_naver_news(query, s_date, e_date):
    ...
    for i in dt_list:        
        print(i)
        df_one = search_naver_news_alldays(query, i, i)
...
def search_naver_news_alldays(query, s_date, e_date):
    ...
    while True:
        ...
        url = "https://search.naver.com/search.naver?where=news&query=" + query + "&sort=1&ds=" + s_from + "&de=" + e_to + "&nso=so%3Ar%2Cp%3Afrom" + s_from + "to" + e_to + "%2Ca%3A&start=" + str(page)
        #header 추가
        header = {
        ...
        news_list = news_list +[i["href"] for i in soup.select("._sp_each_url") if i["href"].startswith("https://news.naver.com")]
...
def get_rank_news(cdate = date.today()):
    ...
    for i in rank_type:
        day_url= 'https://news.naver.com/main/ranking/popularDay.nhn?rankingType=popular_day&sectionId=' + i + '&date='+cdate
        r=requests.get(day_url)
        cont=BeautifulSoup(r.content,"html.parser")    
        rank_list = cont.find_all("div", class_="ranking_headline")
        ...
        for j in rank_list[:5]:
            href = default_url + j.find("a")['href']
            title = j.find("a")['title']    
            ranknews_df = ranknews_df.append(pd.DataFrame([[aclass, href,  title]],
                                                             columns=columns), ignore_index=True)
...
def get_week_rank(cdate = date.today()):
    ...
    with Pool(7) as p:
        get_news_list = p.map(get_rank_news, day_list)
